...
export const api = {
  ...
  // Economy & Banking
  economyRegionList: () => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'region_list' }) }),
  economyRegionUpdate: (region, data) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'region_update', region, ...data }) }),
  economyRoutesList: () => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'routes_list' }) }),
  economyRouteCreate: (data) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'route_create', ...data }) }),
  economyBanksList: () => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'banks_list' }) }),
  economyBankCreate: (data) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'bank_create', ...data }) }),
  economyMyLoans: () => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'my_loans' }) }),
  economyLoanRequest: (bankId, principal, termDays) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'loan_request', bank_id: bankId, principal, term_days: termDays }) }),
  economyLoanRepay: (loanId, amount) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'loan_repay', loan_id: loanId, amount }) }),
  economyLoanList: () => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'loan_list' }) }),
  economyEventTrigger: (data) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'event_trigger', ...data }) }),
  economyEventList: (region) => request('web_economy.php', { method: 'POST', body: JSON.stringify({ action: 'event_list', region }) }),

  // Diplomacy & Treaties
  diplomacyTreatyList: (houseId, status) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'treaty_list', house_id: houseId, status }) }),
  diplomacyTreatyPropose: (data) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'treaty_propose', ...data }) }),
  diplomacyTreatySign: (treatyId, expiresAt) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'treaty_sign', treaty_id: treatyId, expires_at: expiresAt }) }),
  diplomacyTreatyBreak: (treatyId, brokenReason) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'treaty_break', treaty_id: treatyId, broken_reason: brokenReason }) }),
  diplomacyMarriageList: (houseId, status) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'marriage_list', house_id: houseId, status }) }),
  diplomacyMarriagePropose: (data) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'marriage_propose', ...data }) }),
  diplomacyMarriageAccept: (marriageId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'marriage_accept', marriage_id: marriageId }) }),
  diplomacyMarriageCeremony: (marriageId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'marriage_ceremony', marriage_id: marriageId }) }),
  diplomacyMarriageBreak: (marriageId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'marriage_break', marriage_id: marriageId }) }),
  diplomacyHostageList: (houseId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'hostage_list', house_id: houseId }) }),
  diplomacyHostageTake: (data) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'hostage_take', ...data }) }),
  diplomacyHostageRelease: (hostageId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'hostage_release', hostage_id: hostageId }) }),
  diplomacyRelationsList: (houseId) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'relations_list', house_id: houseId }) }),
  diplomacyRelationsUpdate: (data) => request('web_diplomacy.php', { method: 'POST', body: JSON.stringify({ action: 'relations_update', ...data }) }),

  // Combat/Magic Expansion
  magicMyAbilities: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'my_abilities' }) }),
  magicAbilityUnlock: (avatarKey, abilityType) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'ability_unlock', avatar_key: avatarKey, ability_type: abilityType }) }),
  magicAbilityUse: (abilityType) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'ability_use', ability_type: abilityType }) }),
  magicWargingStart: (targetType, targetName, durationSecs) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'warging_start', target_type: targetType, target_name: targetName, duration_secs: durationSecs }) }),
  magicWargingEnd: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'warging_end' }) }),
  magicWargingActive: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'warging_active' }) }),
  magicVisionReceive: (data) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'vision_receive', ...data }) }),
  magicVisionList: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'vision_list' }) }),
  magicVisionInterpret: (visionId, interpretation) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'vision_interpret', vision_id: visionId, interpretation }) }),
  magicRitualStart: (data) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'ritual_start', ...data }) }),
  magicRitualComplete: (ritualId, result, notes) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'ritual_complete', ritual_id: ritualId, result, result_notes: notes }) }),
  magicRitualList: (status) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'ritual_list', status }) }),
  magicDragonglassRecipes: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'dragonglass_recipes' }) }),
  magicDragonglassCraft: (recipeId) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'dragonglass_craft', recipe_id: recipeId }) }),
  magicValyrianRecipes: () => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'valyrian_recipes' }) }),
  magicValyrianReforge: (recipeId) => request('web_magic.php', { method: 'POST', body: JSON.stringify({ action: 'valyrian_reforge', recipe_id: recipeId }) }),

  // World Map & Travel
  travelConnectionsFrom: (territoryId) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'connections_from', territory_id: territoryId }) }),
  travelConnectionsAll: () => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'connections_list_all' }) }),
  travelConnectionCreate: (data) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'connection_create', ...data }) }),
  travelJourneyStart: (fromId, toId, escortCount) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'journey_start', from_territory_id: fromId, to_territory_id: toId, escort_count: escortCount }) }),
  travelJourneyStatus: () => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'journey_status' }) }),
  travelJourneyHistory: () => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'journey_history' }) }),
  travelJourneyArrive: () => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'journey_arrive' }) }),
  travelJourneyCancel: () => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'journey_cancel' }) }),
  travelEncounterList: (journeyId) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'encounter_list', journey_id: journeyId }) }),
  travelEncounterTrigger: (data) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'encounter_trigger', ...data }) }),
  travelEncounterResolve: (encounterId, resolution) => request('web_travel.php', { method: 'POST', body: JSON.stringify({ action: 'encounter_resolve', encounter_id: encounterId, resolution }) }),

  // Event Calendar
  calendarEventList: (all, type, region) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_list', all, event_type: type, region }) }),
  calendarEventDetail: (eventId) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_detail', event_id: eventId }) }),
  calendarEventCreate: (data) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_create', ...data }) }),
  calendarEventCancel: (eventId) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_cancel', event_id: eventId }) }),
  calendarEventRegister: (eventId) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_register', event_id: eventId }) }),
  calendarEventUnregister: (eventId) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_unregister', event_id: eventId }) }),
  calendarMyEvents: () => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'my_events' }) }),
  calendarEventRegistrations: (eventId) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_registrations', event_id: eventId }) }),
  calendarEventAttend: (eventId, avatarKey) => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_attend', event_id: eventId, avatar_key: avatarKey }) }),
  calendarPendingReminders: () => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'pending_reminders' }) }),
  calendarUpcomingCount: () => request('web_calendar.php', { method: 'POST', body: JSON.stringify({ action: 'event_upcoming_count' }) }),
}
